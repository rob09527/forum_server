/**
 * worker 阶段 2 造数(原 scripts/import-nodeloc/fabricate.ts 迁成函数):给已导入的
 * NodeLoc 影子内容补齐「社区运营痕迹」。由 import-sync 状态机在回填完成后自动执行,
 * 不再有 CLI 入口 / --reset / process.argv(需要重造时用 reset-dev-db.ts 清库重来)。
 *
 ...
 *    本模块一次性载入等级表,在内存里连 balanceAfter 链,最后每人一次聚合更新,
 ...
 *    且影子账号没人登录,通知没有消费方。
 *
 * 幂等性:检测到影子用户已有积分流水就**直接返回**(worker 场景下这是「已完成」信号,
 * 不是错误),避免二次重放把余额翻倍。
 */

import { prisma } from '../../lib/prisma.js'
import { INCOME_POINT_TYPES, PointType } from '../../constants/business.js'
import { getLevels, getTipConfig } from '../config/config.service.js'
import { levelForTotal, POINT_RULES } from '../points/points.service.js'
...
// 进程内可变状态:worker 进程跨轮复用,必须在每轮 runFabricate 开头重置(见下),
// 否则第二次调用会残留上一轮的旧流水、PRNG 不复现。
...
let logsByUser = new Map<number, PendingLog[]>()

let states = new Map<number, UserState>()
...
function pushLog(
  ...
  st.balance += delta
  if (INCOME_POINT_TYPES.has(type)) st.totalEarned += delta
...
async function replayPoints(shadow: ShadowSet): Promise<void> {
  ...
  console.log(
    `[import-fabricate] ① 积分重放:${posts.length} 帖 + ${comments.length} 评论 → ${[...logsByUser.values()].reduce((s, l) => s + l.length, 0)} 条流水`,
  )
}
...
async function fabricateFollows(shadow: ShadowSet): Promise<number> {
  ...
  console.log(`[import-fabricate] ② 关注:${candidates.length} 组互动 → ${rows.length} 条关注`)
  return rows.length
}
...
async function fabricateTips(shadow: ShadowSet): Promise<number> {
  ...
  console.log(
    `[import-fabricate] ③ 打赏:${candidates.length} 个候选内容 → ${tipRows.length} 笔打赏,${heatDelta.size} 篇帖子加热度`,
  )
...
async function fabricateShopPurchases(shadow: ShadowSet): Promise<number> {
  ...
  if (items.length === 0) {
    console.log('[import-fabricate] ④ 消费:shop_items 无可售商品,跳过')
    return 0
  }
  ...
  console.log(`[import-fabricate] ④ 消费:${decorRows.length} 位用户买入并佩戴装扮`)
...
async function persist(): Promise<void> {
  ...
  console.log(`[import-fabricate] 落库:${written} 条流水,${states.size} 位用户余额/累计/等级已更新`)
}

// ── 入口 ────────────────────────────────────────────────────────────

/**
 * 执行一次造数(由 import-sync 状态机在回填完成后调用)。
 * 幂等:影子用户已有积分流水 → 视为已造数,直接返回(状态机会据此切到 incremental)。
 */
export async function runFabricate(assertLock: () => void): Promise<void> {
  // 进程内可变状态重置:worker 进程跨轮复用,上次跑的 rand/logsByUser/states 必须清空
  ...
  if (shadow.active.length === 0) {
    throw new Error('未找到影子用户(import_user_mappings 为空),无法造数')
  }
  console.log(
    `[import-fabricate] 影子用户 ${shadow.active.length} 位,占位账号 ${shadow.placeholders.size} 个(全额豁免)`,
  )

  // 幂等守卫:已有流水说明造过数(或增量 worker 已发过分)。worker 场景下这是「已完成」信号,
  // 直接返回而不是 throw —— 状态机会据此把阶段推进到 incremental。
  const existing = await prisma.pointLog.count({
    where: { userId: { in: [...shadow.active, ...shadow.placeholders] } },
  })
  if (existing > 0) {
    console.log(`[import-fabricate] 影子用户已有 ${existing} 条积分流水,视为已造数,跳过`)
    return
  }
  ...
  console.log('[import-fabricate] 完成')
}
